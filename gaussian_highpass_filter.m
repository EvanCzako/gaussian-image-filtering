function im_2 = gaussian_highpass_filter(im_1,low_freq,show_freq_domain,y_over_x_scale_factor)
    
    % Returns a highpass-filtered version of im_1. Applies an inverted 
    % Gaussian filter to im_1 by acquiring its frequency domain and setting  
    % the full-width-at-half-max of the Gaussian filter to the spatial 
    % frequency defined by low_freq. Spatial frequency intensity decays as 
    % spatial frequency approaches 0 according to the filter.
    ...
    % Example 1:
    %
    %     % Apply a Gaussian highpass filter to image 'trees.tif'.
    %     % Currently the lower frequency limit is set to 25, corresponding to a
    %     % spatial frequency of 25 Hz in both the x- and y-directions.
    ...
    %
    %
    % See also: gaussian_lowpass_filter
    %
